...
export default function Chatbot({ onResult }) {
  const [messages, setMessages] = useState([
    {
      role: "system",
      text: "Ask about deforestation, vegetation change, or urban expansion using natural language."
    }
  ]);
  ...
  async function sendQuery() {
    ...
    try {
      ...
      const resp = await axios.post("http://localhost:3333/api/query", {
        text: input
      });
      ...
      setMessages((m) => [
        ...
        {
          role: "assistant",
          text: "Error running query. Check backend console."
        }
      ]);
    } finally {
      setLoading(false);
